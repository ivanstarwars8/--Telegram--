...
def plan_switch_policy_keyboard(level: str, reason: str) -> InlineKeyboardMarkup:
    buttons = [
        [InlineKeyboardButton(text="Перенести всё", callback_data=f"plan:switch_policy:{level}:{reason}:carry_all")],
        [InlineKeyboardButton(text="Сжать до 50%", callback_data=f"plan:switch_policy:{level}:{reason}:compress_50")],
        [InlineKeyboardButton(text="Отменить рутину", callback_data=f"plan:switch_policy:{level}:{reason}:cancel_routine")],
    ]
    return InlineKeyboardMarkup(inline_keyboard=buttons)
...
def focus_session_keyboard() -> InlineKeyboardMarkup:
    buttons = [
        ...
        [InlineKeyboardButton(text="Готово", callback_data="task:done"), InlineKeyboardButton(text="Пропуск", callback_data="task:skip")],
        [InlineKeyboardButton(text="Следующая", callback_data="task:next"), InlineKeyboardButton(text="Список", callback_data="task:list")],
